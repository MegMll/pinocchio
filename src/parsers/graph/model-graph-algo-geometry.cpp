...
namespace pinocchio
{
  namespace graph
  {
    struct AddGeometryToModel : public boost::static_visitor<>
    {
        ...
        {
          addGeometry(std::shared_ptr<fcl::CollisionGeometry>(new fcl::Capsule(c.size[0], c.size[1])));
        }

        void operator()(const Cylinder & c) 
        {
          addGeometry(std::shared_ptr<fcl::CollisionGeometry>(new fcl::Cylinder(c.size[0], c.size[1])));
        }

        void operator()(const Box & b) 
        {
          addGeometry(std::shared_ptr<fcl::CollisionGeometry>(new fcl::Box(b.size[0], b.size[1], b.size[2])));
        }
        ...
        void operator()(const Mesh & m)
        {
          ...
          GeometryObject geometry_object(
            geom.name, body_frame.parentJoint, f_id, body_frame.placement * geom.placement, bvh, m.path,
            geom.scale, true, geom.color);
          ...
          if(f.type == BODY)
          {
            ...
            if (vertex == g.name_to_vertex.end())
              PINOCCHIO_THROW_PRETTY(
                std::invalid_argument, "Graph - Body in model is not in the graph. Please check arguments.");
            ...
            {
              AddGeometryToModel add_geom(f, model.getFrameId(f.name, BODY), geom, mesh_loader, geomModel);
              if(type == VISUAL && (geom.type == GeomType::VISUAL || geom.type == GeomType::BOTH))
                boost::apply_visitor(add_geom, geom.geometry);
              
              else if(type == COLLISION && (geom.type == GeomType::COLLISION || geom.type == GeomType::BOTH))
                boost::apply_visitor(add_geom, geom.geometry);
            }
          }
        }
