...
namespace pinocchio
{
  namespace impl
  {
        ...
        CrbaWorldConventionBackwardStep<Scalar, Options, JointCollectionTpl>>
    {
      ...
      template<
        ...
        typename boost::disable_if<
          std::is_same<JointModel, JointModelMimicTpl<Scalar, Options, JointCollectionTpl>>,
          int>::type = 0>
      ...
      template<
        typename JointModel = JointModelMimicTpl<Scalar, Options, JointCollectionTpl>,
      ...
      const Eigen::MatrixBase<ConfigVectorType> & q)
    {
      ...
      for (int i = 0; i < model.mimic_joints.size(); i++)
      {
        Pass3::run(
          model.joints[model.mimic_joints[i]], data.joints[model.mimic_joints[i]],
      ...
      const Eigen::MatrixBase<ConfigVectorType> & q)
    {
      ...
      for (int i = 0; i < model.mimic_joints.size(); i++)
      {
        Pass3::run(model.joints[model.mimic_joints[i]], typename Pass3::ArgsType(model, data));
      }
